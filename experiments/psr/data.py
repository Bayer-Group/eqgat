...
DATA_DIR = osp.join(PATH, "split-by-year/data")
...
class GNNTransformPSR(object):
    ...
    def __call__(self, item: Dict) -> Data:
        ...
        graph.y = torch.FloatTensor([labels["gdt_ts"]])
        split = item["id"].split("'")
...
if __name__ == "__main__":
    ...
    parser = ArgumentParser(
        description="Dataloading script to access the PSR Graph Dataset directly from LMDB."
    )
    parser.add_argument(
        "--cutoff",
