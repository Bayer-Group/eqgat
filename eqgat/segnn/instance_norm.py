...
class InstanceNorm(nn.Module):
    ...
    def __init__(self, irreps, eps=1e-5, affine=True, reduce='mean', normalization='component'):
        ...
        if not isinstance(reduce, str):
            print("reduce should be passed as a string value")
            exit()

        if not reduce in ['mean', 'max']:
            print("reduce needs to be 'mean' or 'max'")
            exit()

        self.reduce = reduce

        if not normalization in ['norm', 'component']:
            print("normalization needs to be 'norm' or 'component'")
            exit()
