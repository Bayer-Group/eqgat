...
Official Implementation from https://github.com/drorlab/gvp-pytorch

    This code is mainly taken from: https://github.com/drorlab/gvp-pytorch/blob/main/gvp/__init__.py
...
def split(x, nv):
    ...
    Should be used only with `merge(s, V)` and only if the tuple
    representation cannot be used.

    :param x: the `torch.Tensor` returned from `merge`
    :param nv: the number of vector channels in the input to `merge`
    """
...
class GVP(nn.Module):
    ...
    def __init__(
        ...
        activations: Tuple[Callable, Callable] = (F.relu, torch.sigmoid),
    ...
    def forward(self, x: Tuple[Tensor, OptTensor]):
        """
        :param x: tuple (s, V) of `torch.Tensor` and OptTensor,
        s has shape (*, n_scalar), if V is a Tensor, the shape is (*, n_vector, 3)
        :return: tuple (s, V) of `torch.Tensor` and OptTensor
        """
        if self.vi:
            s, v = x
            v = torch.transpose(v, -1, -2)
            # apply transformation on feature-channels of length `n_vector`
            vh = self.wh(v)
            # to merge the vector features with scalar features, normalize along spatial dimension
            vn = norm_no_nan(vh, axis=-2)
            s = self.ws(torch.cat([s, vn], -1))
            if self.vo:
                # in case we want to obtain vector feature based on a single-node object, further linearly transform
                v = self.wv(vh)
                v = torch.transpose(v, -1, -2)
                if self.vector_gate:
                    if self.vector_act:
            ...
            s = self.ws(x)
            if self.vo:
                # might be for the initial layer of graph convolution where no vector-features are available
                v = torch.zeros(s.shape[0], self.vo, 3, device=self.dummy_param.device)
        if self.scalar_act:
            s = self.scalar_act(s)

        return (s, v) if self.vo else (s, None)
...
class GVPConv(MessagePassing):
    ...
    def forward(self, x, edge_index, edge_attr):
        ...
        x_s, x_v = x
        # x_s
        # for message passing, reshape vector embeddings into a scalar embedding
